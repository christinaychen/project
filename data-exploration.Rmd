...
els %>%
  filter(!is.na(F1A34A)) %>%
  ggplot() +
  geom_point(aes(y=F3ERN2011, x=F1A34A), alpha=0.1)
...
# els%>% ggplot() + geom_bar(aes(x=FREE_LUNCH_PERC)) + facet_grid(.~SCHOOL_TYPE)

els %>%
  filter(!is.na(FREE_LUNCH_PERC)) %>%
  ggplot() +
  geom_boxplot(aes(y=BYTXCSTD, x=FREE_LUNCH_PERC))

els %>%
  filter(!is.na(FREE_LUNCH_PERC)) %>%
  ggplot() +
  geom_boxplot(aes(y=F3ERN2011, x=FREE_LUNCH_PERC))

els %>%
  filter(!is.na(FREE_LUNCH_PERC), !is.na(ED_ATNMT)) %>%
  ggplot() +
  geom_bar(aes(x=ED_ATNMT, fill=FREE_LUNCH_PERC), position="fill")
